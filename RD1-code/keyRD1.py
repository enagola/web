...
LEFT  = ['turn left' , 'left' , 'left left', 'turn lamp']
RIGHT = ['turn right' , 'right' , 'right right','sunlight','turn bright','turn cry','contrite']
FORWARD  = ['go forward' ,'forward','go', 'forward forward' , 'go go', 'go go go']
BACKWARD = ['go back','back' ,'go backward', 'backward','back back','back work']
...
try:
    while True:      
        ...
        if char == ord('q'):
           ...
           if(voice_command):
                if voice_command == 'i': 
                    screen.addstr(0, 0, voice_command)         
                    port.write('i'.encode()); 
                    time.sleep(4)  
                    port.write('s'.encode());   
                    ...
                    screen.addstr(0, 0, 'no voice_command')   
